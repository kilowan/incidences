...
export default {
  ...
  methods: {
    save()
    {
      axios({
        method: 'get',
        url: 'http://localhost:8082/newMenu.php?funcion=updateEmployee&dni=' + this.userData.dni + '&name=' + this.userData.surname1 + '&surname1=' + this.userData.surname2 + '&surname2=' + this.userData.surname2 + '&type=' + this.userData.type,
        headers:[],
      }).then(
        this.$emit('reload')
      );
